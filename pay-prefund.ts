/**
 * Gaian Network - Prefund Payment Script (Testnet/Sandbox)
 *
 * Usage: bun run pay-prefund.ts
 *
 * Flow:
 *   1. Create prefund order → order auto-proceeds (no on-chain payment needed)
 *   2. Script polls status until completion
 */
...
interface PlaceOrderPrefundResponse {
	orderId: string;
	status: string;
	fiatAmount: number;
	fiatCurrency: string;
...
async function createPrefundOrder(
	apiKey: string,
): Promise<PlaceOrderPrefundResponse> {
	console.log("\n━━━ Step 1: Create Prefund Order ━━━\n");
	...
	const cryptoCurrency = prompt(
		"Crypto currency (USDC / USDT): ",
	).toUpperCase();
	if (!["USDC", "USDT"].includes(cryptoCurrency)) {
	...
	const transactionReference = prompt(
		"Transaction reference / memo (optional, press Enter to skip): ",
	);
	...
	const order = await apiPost<PlaceOrderPrefundResponse>(
		"/api/v1/placeOrder/prefund",
		body,
		apiKey,
	);

	console.log("\n┌─────────────────────────────────────");
	console.log("│ PREFUND ORDER CREATED");
	console.log("├─────────────────────────────────────");
	console.log(`│ Order ID:        ${order.orderId}`);
	console.log(`│ Status:          ${order.status}`);
	console.log(`│ Prefunded:       ${order.isPrefunded}`);
...
async function main() {
	console.log("╔═══════════════════════════════════════╗");
	console.log("║    Gaian Network Prefund Payment      ║");
	console.log("╚═══════════════════════════════════════╝\n");
	...
	// Step 1: Create prefund order (auto-proceeds, no on-chain payment needed)
	const order = await createPrefundOrder(apiKey);
	...
	// Step 2: Poll status (order auto-proceeds since it's prefunded)
